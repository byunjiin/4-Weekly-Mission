...
export default function SigninPage() {
  const {
    ...
    clearErrors,
    setError,
  } = useForm<signinForm>({ mode: "onBlur", reValidateMode: "onBlur" });
  const router = useRouter();

  const signInCheck = async (data: signinForm) => {
    try {
      const signInAPI = await axios.post(
        "https://bootcamp-api.codeit.kr/api/sign-in",
        data
      );
      if (signInAPI.status === 200) {
        // 모든 오류가 없을때
  ...
  const onSubmitHandler: SubmitHandler<signinForm> = async (data) => {
    signInCheck(data);
  };
